...
// Si no existe el usuario, redirige a la página de añadir usuarios
if (!$usuario) {
    header("Location: addUser.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Si se ejecuta correctamente la consulta, mostrará en verde un mensaje de éxito
    if ($stmt->execute()) {
        $success = "Usuario actualizado correctamente.";

        // Volvemos a hacer SELECT para obtener la info actualizada del usuario
        $stmt = $mysqli->prepare("SELECT nombre_usuario, email, rol FROM USUARIOS WHERE id=?");
        $stmt->bind_param("i", $usuario_id);
        $stmt->execute();
        $usuario = $stmt->get_result()->fetch_assoc();
...
<div class="container my-5">
    ...
    <!-- Enviamos por POST el nombre de usuario, email, contraseña y rol -->
    <form method="POST">
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="rol" id="rol" required>
                ...
                <!-- Que salga Administrador si es el rol actual -->
